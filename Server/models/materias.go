...
func (m *Materia) GetAllMateriasByCarrera(db *pgx.Conn) ([]Materia, error) {
	...
	for rows.Next() {
		var m Materia
		err := rows.Scan(&m.Codigo, &m.Nombre, &m.Info, &m.Uc, &m.Horas_estudio, &m.Electiva, &m.Departamento, &m.Nucleo, &m.Semestre, &m.UC_requeridas, &m.Correquisito, &m.Prelaciones, &m.Debloqueables)
		if err != nil {
			log.Printf("Error fetching Materias: %v", err)
			return materias, err
		}
